...
std::vector<Float_t> TrueE_vec;
std::vector<Float_t> NPhotons_vec;
...
struct Flash_info {
      ...
      std::vector<Float_t> hit_vectorALL;
      std::vector<Float_t> hitch_vectorALL;
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH1F* hist = new TH1F("hist","Resolution for 2D reconstruction", 100, 0, 2200);
   TH1F* hist1 = new TH1F("hist1","Resolution for 2D reconstruction", 100, 0, 2200);
   TH1F* hist2 = new TH1F("hist2","Resolution for 2D reconstruction", 100, 0, 2200);
   TH1F* hist3 = new TH1F("hist3","Resolution for 3D reconstruction", 100, 0, 2200);
   TH1F* hist4 = new TH1F("hist4","Resolution for 3D reconstruction", 100, 0, 2200);
   TH1F* hist5 = new TH1F("hist5","Resolution for 3D reconstruction", 100, 0, 2200);
   TH1F* hist6 = new TH1F("hist6","Error 0 points #PEs", 100, 0, 2200);
   TH1F* hist7 = new TH1F("hist7","Time profile", 100, 0, 2200);
   TH1F* hist8 = new TH1F("hist8","2D clustering resolution", 100, 0, 2200);
   TH1F* hist9 = new TH1F("hist9","gaussian resolution", 100, 0, 2200);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      double array1[Channel_Xordered->size()];
      double array2[Channel_Xordered->size()];
      double array3[Channel_Xordered->size()];
	 ...
	 flash.hit_vectorALL.push_back(OpHitPE_Flash->at(j));
	 flash.hitch_vectorALL.push_back(OpHitChannel_Flash->at(j));

	 flash.standard_X = RecoXVector->at(0);
	 flash.standard_Y = YCenterVector->at(0);
	 flash.standard_Z = ZCenterVector->at(0); 
...
//             flash.w_sumY += Channel_Y->at(j)*OpHitPE_Flash->at(j);
         ...
         flash.old_sumweight += OpHitPE_Flash->at(j);
	      ...
	      for (int l=0; l<flash.hitch_vectorALL.size();l++)
		{
		  Fitter.Addhit(flash.hitch_vectorALL.at(l), flash.hit_vectorALL.at(l));
		}
	      std::cout<<"fit"<<std::endl;
	      Fitter.Fit();
	      ROOT::Fit::FitResult result = Fitter.FitResult();
          TrueE_vec.push_back(TrueE*1000);
          NPhotons_vec.push_back(result.GetParams()[3]);
//           hist8 -> Fill(abs(result.GetParams()[0] - TrueX));
//           hist9 -> Fill(abs(result.GetParams()[1] - TrueY));
//           hist -> Fill(abs(result.GetParams()[2] - TrueZ));
//
...
// std::cout << "Cathode Y: " << w_meanYC << " " << TrueY << std::endl;
	 ...
	 std::cout<<" TRUE pos: "<<TrueX<<" "<<TrueY<<" "<<TrueZ<<std::endl;
	 std::cout<<" standard method "<<flash.standard_X<<" "<<flash.standard_Y<<" "<<flash.standard_Z<<std::endl;
...
//    std::cout << w_meanY << std::endl;
   ...
   TGraph *gr4 = new TGraph(TrueE_vec.size(), &TrueE_vec[0], &NPhotons_vec[0]);
   gr4 -> SetTitle("Solid angle reconstruction, #PEs vs True E");
   gr4 -> GetXaxis() -> SetTitle("True Energy (MeV)");
   gr4 -> GetYaxis() -> SetTitle("Number of photoelectrons");
   gr4 -> Draw("AP");

//       hist -> GetXaxis()->SetTitle("Resolution (cm)");
//       hist -> SetTitle("Solid angle reconstruction");
//       hist -> Draw();
//       hist9 -> GetXaxis()->SetTitle("Resolution (cm)");
//       hist9 -> Draw("SAME");
//       hist9 -> SetLineColor(kRed);
//       hist8 -> SetLineColor(kGreen);
//       hist8 -> GetXaxis()->SetTitle("Resolution (cm)");
//       hist8 -> Draw("SAME");
...
//    legend1 -> AddEntry(hist8, Form("New mean X:  %5.2f (cm)", hist8->GetMean())); //Form("New mean: " %5.2f, std::to_string(hist->GetMean()).c_str())
//    legend1 -> AddEntry(hist9, Form("New mean Y:  %5.2f (cm)", hist9->GetMean()));
//    legend1 -> AddEntry(hist, Form("New mean Z:  %5.2f (cm)", hist->GetMean()));
//    legend1 -> Draw();
...
//    hist2D1 -> Draw("COLZ");
//    TGraph *gr3 = new TGraph(TrueX_vector_midE.size(), &TrueX_vector_midE[0], &RMS_vectorY_midE[0]);
