...
//    return {flashes};
   ...
   while(flashes.size() != 0){
      ...
      for(auto& cluster : clusters){
         if (abs(cluster[0].flash_time - new_flash.flash_time) <= 0.5 && abs(cluster[0].meanZ - new_flash.meanZ) <= 400){
...
//          oldw_wmeanZ = flash.oldw_sumZ/flash.old_sumweight;
            ...
            if (flash.plane == kC){
               hist8 -> Fill(flash.channel_counter);
            }
            else{
               hist9 -> Fill(flash.channel_counter);
            }
...
//       std::cout << "Nb clusters -> " << flash_clusters.size() << std::endl;
      ...
      for(auto const& cluster : flash_clusters){
         ...
         if (combined_x !=0 && combined_y != 0 && combined_z != 0){ //can change which coordinate I use to allow single flashes in specific planes (x for cathode and y for walls)
            for(Flash_info const& flash: cluster){
...
//                   hist6 -> Fill(flash.sumC + flash.sumL +flash.sumR + flash.sumback + flash.sumfront);
   ...
   // gr4 -> Draw("AP");
      ...
      // hist2D4 -> SetTitle("Reconstructed signal flashes, XZ plane, flash cut");
      // hist2D4 -> Draw("COLZ");
         ...
         //  hist2D6 -> Draw("COLZ");
      hist8 -> SetTitle("#X-ARAPUCA hit per flash for background, cathode");
      hist8 -> GetXaxis()->SetTitle("#X-ARAPUCA hit");
      hist8 -> Draw();
...
//    gr3 -> Draw("AP");
         ...
         // hist2D5 -> SetTitle("Reconstructed signal flashes, YZ plane, flash cut");
         // hist2D5 -> Draw("COLZ");
      // hist7 -> Scale(scale);
      // hist7 -> SetTitle("#NPEs per drift window, background");
      // hist7 -> GetXaxis()->SetTitle("#PEs");
      // hist7 -> Draw("HIST");
      hist9 -> SetTitle("#X-ARAPUCA hit per flash for background, walls");
      hist9 -> GetXaxis()->SetTitle("#X-ARAPUCA hit");
      hist9 -> Draw();
