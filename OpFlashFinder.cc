...
namespace opdet {
 ...
 std::vector<int> OpFlashFinderVerticalDrift::getNeighbors( std::vector< recob::OpHit > const&       HitVector,int hitnumber, std::vector<bool> &processed, float initimecluster, std::vector<int> &sorted,geo::GeometryCore const& geom)
 {
  ...
  for (unsigned int h=hitnumber;h<HitVector.size();h++) if(HitVector[sorted[h]].PeakTime() > HitVector[sorted[hitnumber]].PeakTime()+fMaximumTimeDistance)
  {
   ...
   std::cout <<"Earliest: "<< HitVector[sorted[hitnumber]].PeakTime() <<" " << xyz_hitnumber[0] << " " << xyz_hitnumber[1] << " " << xyz_hitnumber[2] << std::endl;
   ...
   for (int h=itTmin;h<itTmax;h++)
   {
     if(!processed[h])
     {
       ...
       std::cout << "Next:    " << HitVector[sorted[h]].PeakTime() << " "<< xyz_h[0] << " " << xyz_h[1] << " " << xyz_h[2] << std::endl; 
       ...
       if(xyz_h[0] < fCx && xyz_hitnumber[0] < fCx){
       ...
       if(xyz_h[1] > fRMy && xyz_hitnumber[1] > fRMy){
       ...
       if(xyz_h[1] < fLMy && xyz_hitnumber[1] < fLMy){
         if(dt<fMaximumTimeDistance && dtmax<fMaximumTimeWindow && dist < fMMdz){ neighbors.push_back(h);processed[h]=true;}
         std::cout <<" L L "<< fMMdz <<" "<< dist << std::endl;
       }
       if(TMath::Abs(xyz_h[1]) > fRMy && xyz_hitnumber[0] < fCx){
         dist = TMath::Abs(dz);
       ...
       if(xyz_h[0] < fCx && TMath::Abs(xyz_hitnumber[1]) > fRMy){
         dist = TMath::Abs(dz);
         if(dt<fMaximumTimeDistance && dtmax<fMaximumTimeWindow && dist < fCMdz){ neighbors.push_back(h);processed[h]=true;}
         std::cout <<" RL C "<< fCMdz <<" "<< dist << std::endl;
       }
       if(xyz_h[1] < fLMy && xyz_hitnumber[1] > fRMy){
         dist = TMath::Abs(dz);
         if(dt<fMaximumTimeDistance && dtmax<fMaximumTimeWindow && dist < fMMdz){ neighbors.push_back(h);processed[h]=true;}
         std::cout <<" R L "<< fMMdz <<" "<< dist << std::endl;
       }
       if(xyz_h[1] > fRMy && xyz_hitnumber[1] < fLMy){
         dist = TMath::Abs(dz);
         if(dt<fMaximumTimeDistance && dtmax<fMaximumTimeWindow && dist < fMMdz){ neighbors.push_back(h);processed[h]=true;}
